...
// 🔑 HX GATE
#define HX_GATE_THRESHOLD 0 // Sent data after this threshold reached
...
// FSR CONFIG
// =================================================
const int fsrPins[3] = {33, 35, 32};   // Right, Centre, Left
const float FORCE_THRESHOLD[3] = {400, 5000, 400};
...
void handleFSR() {
  ...
  if (!systemLocked) {
    ...
    else if (forces[0] > FORCE_THRESHOLD[0]) {
      currentStatus = "RIGHT";
      systemLocked = true;
    }
    else if (forces[2] > FORCE_THRESHOLD[2]) {
      currentStatus = "LEFT";
      systemLocked = true;
    }
  }
...
void loop() {
  ...
  // LEFT / RIGHT should cancel HX immediately
  if ((currentStatus == "LEFT" && lastStatus != "LEFT" || currentStatus == "RIGHT" && lastStatus != "RIGHT") &&
    ...
    Serial.println("⛔ HX CANCELLED (LEFT/RIGHT)");
  }
  ...
  if (eventReady) {
    ...
    // SEND LEFT / RIGHT (no count increase, no pressure change)
  if ((currentStatus == "LEFT" || currentStatus == "RIGHT") &&
      lastStatus != currentStatus) {
  ...
  // CONTINUOUS LIVE PRESSURE SEND (does not affect logic)
  static unsigned long lastLiveSend = 0;

  if (millis() - lastLiveSend > 100) {   // send every 100ms
    lastLiveSend = millis();
    if(currentStatus != "CENTRE") {
      String json = "{";
      json += "\"pressure\":" + String(currentHXValue, 2) + ",";
      json += "\"count\":" + String(centreCount) + ",";
      json += "\"status\":\"" + currentStatus + "\"";
      json += "}";
